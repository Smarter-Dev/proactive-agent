...
class AgentDeps:
    ...
    instruction_stores: dict[str, InstructionStore] = field(default_factory=dict)
    ...
    # evals).
    request_mode: Callable[[str, str, int], str | Awaitable[str]] | None = None
    ...
    drain_notifications: Callable[[], str | Awaitable[str]] | None = None
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError(
                "enabled_channels is required with legacy env or instruction_store"
            )
        if (self.env is not None or self.instruction_store is not None) and len(
            self.enabled_channels
        ) != 1:
            raise ValueError(
                "legacy env or instruction_store requires exactly one enabled channel"
            )
        ...
        if self.instruction_store is not None and not self.instruction_stores:
            self.instruction_stores = {legacy_channel_id: self.instruction_store}
        ...
        if self.instruction_store is None:
            self.instruction_store = next(iter(self.instruction_stores.values()), None)
    ...
    *,
    ...
    async def send_channel_message(
        ...
        ctx.deps.actions.sent.append(
            ProposedResponse(reply_to_id=None, content=content, channel_id=channel_id)
        )
        return "Message sent."
    ...
    async def react_to_message(
        ...
        ctx.deps.actions.reactions.append(
            ProposedReaction(message_id=message_id, emoji=emoji, channel_id=channel_id)
        )
    ...
    async def read_notifications(ctx: RunContext[AgentDeps]) -> str:
        ...
        result = ctx.deps.drain_notifications()
    ...
    async def set_monitoring_mode(
        ...
        result = ctx.deps.request_mode(channel_id, mode, minutes)
        return await result if isawaitable(result) else result
...
async def self_compaction_summary(
    ...
    result = await compaction_agent.run(COMPACTION_PROMPT, message_history=messages)
