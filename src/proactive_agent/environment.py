...
from proactive_agent.transcript import (
...
class InstructionStore:
    ...
    def clear_instruction(self, instruction_id: str) -> bool:
        remaining = [e for e in self.entries if e.instruction_id != instruction_id]
    ...
    def prune_expired(self, now: datetime | None = None) -> list[WatchInstruction]:
        now = now or datetime.now(UTC)
        ...
        try:
            ...
            store.entries.append(
                WatchInstruction(
                    instruction_id="w1",
                    text=stored,
                    expires_at=now + timedelta(seconds=LEGACY_ADDENDUM_TTL_SECONDS),
                )
            )
            store._next_id = 2
            return store
